# 🇩🇪 German A1 — Complete Curriculum (Grammatik Aktiv Edition)

> **Target Exam:** Goethe-Zertifikat A1: Start Deutsch 1
> **Based on:** *Grammatik aktiv A1-B1* (Cornelsen) — Unit-by-unit alignment
> **Language:** English instruction with Vietnamese notes (🇻🇳)
> **Duration:** 8 weeks (recommended) | 12 modules

---

## 📚 Course Modules

### Phase 1: Foundation (Weeks 1-2) — Book Units 1-4

| # | Module | Book | Key Grammar |
|---|---|---|---|
| 01 | [Personalpronomen](M01-erste-schritte/) | Kap. 1 | 9 pronouns, Pronomen für Personen/Sachen, sie/sie/Sie |
| 02 | [Konjugation Präsens](M02-pronomen-sein/) | Kap. 2 | Verb endings -e/-st/-t/-en, kommen/arbeiten/heißen |
| 03 | [Sein, haben & Vokalwechsel](M03-haben-possessiv/) | Kap. 3-4 | sein/haben, möchten/wissen/tun, e→i/e→ie/a→ä |

### Phase 2: Modal & Structure (Weeks 3-4) — Book Units 5-12

| # | Module | Book | Key Grammar |
|---|---|---|---|
| 04 | [Modalverben](M04-modalverben/) | Kap. 5-6 | 6 modal verbs, conjugation, infinitive at end, usage distinctions |
| 05 | [Trennbare Verben & Imperativ](M05-trennbar-imperativ/) | Kap. 7-8 | Separable verbs, sentence frame, imperative (du/ihr/Sie) |
| 06 | [Fragen & Wortstellung](M06-fragen-wortstellung/) | Kap. 9-12 | W-questions, yes/no questions, verb position, basic word order |

### Phase 3: Nouns & Cases (Weeks 5-6) — Book Units 13-20

| # | Module | Book | Key Grammar |
|---|---|---|---|
| 07 | [Nomen, Artikel & Plural](M07-nomen-artikel-plural/) | Kap. 13-14 | Gender, articles, 5 plural patterns |
| 08 | [Negation](M08-negation/) | Kap. 15 | kein, nicht, nie, doch |
| 09 | [Akkusativ & Dativ](M09-akkusativ-dativ/) | Kap. 16-17 | Case system, article changes, verbs + case |
| 10 | [Possessivartikel, welcher/dieser & Pronomen](M10-possessiv-pronomen/) | Kap. 18-20 | Possessives, interrogative/demonstrative articles, object pronouns |

### Phase 4: Verb Patterns & Exam Prep (Weeks 7-8) — Book Units 21-22+

| # | Module | Book | Key Grammar |
|---|---|---|---|
| 11 | [Verben mit Akkusativ und Dativ](M11-verben-akkusativ-dativ/) | Kap. 21-22 | Dat+Akk verbs, Dativ verbs, object patterns |
| 12 | [Uhrzeit, Datum & Final Review](M12-uhrzeit-datum-exam/) | Supplementary | Time, dates, final A1 review, Goethe exam prep |

---

## 📂 Each Module Contains

| File | Purpose |
|---|---|
| `lesson.md` | 📖 Grammar theory + examples + dialogues |
| `vocabulary.md` | 📝 Topic-specific word list |
| `exercises.md` | ✍️ Practice drills |
| `answers.md` | 🔍 Complete answer key |
| `exam-tips.md` | 🎯 Goethe A1 strategies |
| `flashcards.md` | 🃏 Spaced repetition cards |

---

## 📎 Supplementary

| Resource | Description |
|---|---|
| [APPENDIX — Goethe Survival Kit](APPENDIX-goethe-survival.md) | Greetings, alphabet, numbers, self-introduction |
| [Curriculum Map](CURRICULUM-MAP.md) | Full CEFR alignment map |

---

## 📌 Conventions

| Symbol | Meaning |
|---|---|
| 🔵 der / 🔴 die / 🟢 das | Gender color coding |
| ⚠️ | Important exception |
| 💡 | Memory tip |
| 🇻🇳 | Vietnamese note |
| 🎯 | Goethe exam tip |
